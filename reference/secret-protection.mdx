...
| --- | --- |
| `command` | Path targets extracted from the parsed command, including segments, pipes, redirection targets, command substitutions, `VAR=value` assignment values, the fallback text of assigning expansions (`${VAR:=path}`, `${VAR=path}`), and the files a `curl` upload flag names |
