...
3. **[Home directory rules](#home-directory-rules)** — whole protected directories such as `~/.ssh`. These take priority over the public-key exemption, so `~/.ssh/id_rsa.pub` still blocks unless a secret allow path covers it.
