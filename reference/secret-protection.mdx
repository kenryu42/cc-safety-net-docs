...
This page is the rule catalog: every built-in rule id, what it matches, and its default state. It is the reference for two questions the other pages defer: *is this file protected?* and *which id turns that rule off?* For the `secret_protection` configuration schema see [Policy](/configuration/policy#secret-protection); for where the stage sits in the guard flow see [Architecture](/guides/architecture#the-ordered-guard-stages); for command-level behavior and worked examples see [Blocked commands](/reference/blocked-commands#sensitive-paths).
...
| --- | --- |
| `command` | Path targets extracted from the parsed command, including segments, pipes, redirection targets, command substitutions, and `VAR=value` assignment values |
