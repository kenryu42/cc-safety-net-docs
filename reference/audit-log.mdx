...
If `command`, `segment`, `toolName`, or `cwd` exceeds its cap, the record gains `truncated: true`. The flag is never written as `false`; its absence means nothing was cut. `logs --id` renders it as `truncated: yes` or `-`.
