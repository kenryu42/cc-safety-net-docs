...
`--check` is no longer accepted. Every subcommand rejects it with `Unknown option for rule <subcommand>: --check`. Rulebooks are read from disk on every command, so an `add` or `update` dry run would have to fetch and validate the candidate to mean anything. [`rule verify`](#rule-verify) is the offline validation command.
...
Usage is `rule add <source> [--ref <ref>] [--only <rulebook...>]`. `<source>` takes three forms: a bare local name such as `project-rules`, a whole repository such as `acme/safety-rules`, or one rulebook in the canonical form `owner/repo#ref/<rulebook-name>`.
...
Omitting the source is an error. `-g`/`--global` selects the user scope, as it does for every other subcommand.
...
A repository add prints, in order:
...
- `Rule config synced.`, a blank line, then `Active rulebooks (<n>):` with `  - <name> <version> (<n> rules)` and `    Source: <spec>` for each

A bare local name or a canonical `owner/repo#ref/<rulebook-name>` source drops the first three lines and closes with `Added rulebook source: <source>` where a repository add says `Rule config synced.`
...
Under `Active rules`, every rule prints the same three rows: `  Command:`, `  Block args:`, and `  Reason:`. A version 1 rule puts `<command> <subcommand>` on the `Command:` row when it sets a subcommand, and its blocked arguments on `Block args:`. A `rulebook_version: 2` rule has neither, so its `Command:` row is the bare command and its `Block args:` row is empty. `rule list` prints nothing from the version 2 `match` contract. Read `match.command_path`, `match.any_args`, and `match.exclude_args` in the rulebook file itself, or run [`rule doc`](#rule-doc) for the authoring guide that describes them.
