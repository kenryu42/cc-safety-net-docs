...
description: "Compare CC Safety Net with OS-level sandboxing. They defend against different threats and provide stronger defense-in-depth when used together."
---
...
- **Reads outside the recognized set.** Sensitive-path protection covers a bounded pattern set — `.env` and its variants, SSH keys, cloud credential stores, coding-CLI credential files, plus the deny paths you configure — across supported command, path, search, and patch shapes; the full catalog is in the [Secret protection reference](/reference/secret-protection). It is not a general read boundary, so a credential in an unrecognized file is not protected. A sandbox constrains *all* reads without knowing which file matters.
