...
description: "Fix common CC Safety Net issues: hook not firing, slow hooks, commands not blocked, false positives, custom rules not enforced, degraded configuration, and status line not showing."
---
...
<AccordionGroup>
  <Accordion title="Hook not firing: commands execute without being checked">
    ...
    **Steps to resolve:**
    ...
    14. **OpenCode**: check `opencode.json` (or `.jsonc`) in OpenCode's config directory, which is `OPENCODE_CONFIG_DIR` when set, otherwise `$XDG_CONFIG_HOME/opencode`, otherwise `~/.config/opencode`. Confirm that the `plugin[]` or `plugins[]` array contains `cc-safety-net`. On OpenCode 2.x, run `opencode plugin list` and confirm a `cc-safety-net` row whose source is `cc-safety-net@latest`; that row means OpenCode found the plugin, not that its hooks activated. On OpenCode 1.x, a stale cached version can keep running: see the [Installation](/installation#opencode-installation) cache-clearance steps.
  ...
  <Accordion title="Hooks are slow: every command waits before it runs">
    A healthy check finishes in well under a second. If every command waits for seconds, the delay is in resolving the `cc-safety-net` package, not in the analysis itself.
    ...
    **Steps to resolve:**
    ...
    4. Repair the npm cache:
       ...
       A bloated or corrupted cache slows every npx resolution, warm runs included. `npm cache verify` garbage-collects and repairs it. In the report that led to this section ([issue #16](https://github.com/kenryu42/cc-safety-net/issues/16)), it reclaimed over 5 GB and brought hooks back under a second.
