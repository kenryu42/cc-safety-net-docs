...
<AccordionGroup>
  ...
  <Accordion title="doctor reports rulebook lock and cache leftovers">
    ...
    `rule sync` is deprecated and does nothing else. It runs offline. It copies any cached rulebook that still matches its recorded digest into the live path its source loads from, then deletes the lock and the cache. For a source it could not migrate it prints one line naming the `rule update` command that vendors it instead. It refuses to run while `rule.json` is missing or unreadable, because the lock would then be the only surviving record of your sources.
  </Accordion>
