...
<Note>
  The package root export is the OpenCode plugin object, not the engine. Import from `cc-safety-net/api` for `checkCommand`. With the OpenCode v1 SDK, import `CCSafetyNetPlugin` or the default plugin's `server` from `cc-safety-net`; with the v2 SDK, use the default export from `cc-safety-net/opencode/v2`. Each entry's declarations require only its own generation's optional peer.
</Note>
