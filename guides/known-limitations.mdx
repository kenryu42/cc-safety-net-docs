...
<Note>
  Standard mode is best-effort for adversarial or dynamic input, and several items below are relaxations that [strict or paranoid mode](/configuration/modes) closes. Where that is the case, it is called out.
</Note>
...
In both cases only the top-level fallback dangerous-text scan may catch the command, and that scan is incomplete. One more nuance: `transparent_wrappers` lives in `rule.json`, so a scope whose `rule.json` is unreadable loses that scope's wrappers until it is fixed — see [Configuration Recovery](/configuration/recovery).
...
The `explain` command shows the full step-by-step analysis of how CC Safety Net evaluated that specific command, including the effective safety level, so you can tell a documented standard-mode relaxation from a real gap.
...
A command shape the rules do not block yet is a **coverage gap**, and it belongs in a public GitHub issue — report the shape, not a weaponized payload. Secret leakage, a write outside the intended directory, and package-integrity problems go through private disclosure instead. The [security policy](/security) has both procedures.
