...
description: "What CC Safety Net cannot catch: opaque command proxies, in-binary behavior, filesystem containment and network, symlink TOCTOU, interpreter flags, quotes inside interpreter comments, eval-based execution, the Codex interactive-session gap, the Grok Build fail-open host, and the Hermes Agent and OpenClaw coverage boundaries."
---
...
If a command does not visibly contain a destructive operation, command-string analysis cannot infer it. `some-tool --task destructive-cleanup` looks benign to CC Safety Net because it cannot inspect what an arbitrary binary does at runtime. Use [sandboxing](/guides/vs-sandboxing) for that threat class.
...
Once configured, analysis looks through the wrapper to the visible child command, so both built-in rules and [custom rules](/configuration/custom-rules) apply to `rtk git reset --hard` as they would to `git reset --hard`. There are no built-in defaults. Configure only wrappers you trust. Reserved commands (`git`, `busybox`, the built-in analyzed commands, shell wrappers, and interpreters) cannot be registered as wrappers.
...
### Quotes inside interpreter comments

Secret protection blanks the string literals in an interpreter's inline code before scanning the rest as code. That pass is lexical and does not skip comments, so a quote character in a comment pairs with the next quote in the code and shifts where literals begin and end. A Ruby or Perl contraction (`# it's the config`) leaves an apostrophe for the next apostrophe to close, and a Python comment containing `"""` opens a triple-quoted string.

A single- or double-quoted literal that reaches the end of a line without closing aborts the pass, and the whole code falls back to the scan that keeps every quoted and path-like token. That catches most mis-pairings. A Python triple-quote opened in a comment has no such stop, so it can hide a read from standard mode. [Strict mode](/configuration/modes#strict-mode-cc_safety_net_strict=1) is unaffected, because it scans inside literal text as well.
...
Tamper resistance covers `policy.json` in both scopes: the user file, and the project file resolved from the execution directory and from the configuration directory. `rule.json` and rulebooks are not protected. Every rulebook is a live file the runtime rereads on each tool call, so an agent that can write in the rules directory changes what is enforced on the next one.
...
Both integrations assume the POSIX state layout. The hosts resolve relocated state through `HERMES_HOME` for Hermes, and through `OPENCLAW_STATE_DIR` followed by the directory of `OPENCLAW_CONFIG_PATH` for OpenClaw. They fall back to `~/.hermes` and `~/.openclaw`. The Windows defaults are not supported. Install and detection target the POSIX path on Windows, so the Hermes install writes the plugin to a directory Hermes does not read. OpenClaw's own CLI installs correctly, but `doctor` misreports the state.
...
Grok Build hooks are fail-open by design, and the host exposes no `failClosed` knob. Only an explicit deny on stdout blocks a tool call. A hook that crashes, times out, or emits malformed output lets the call proceed.

The adapter still emits an explicit deny for its own fail-closed outcomes, such as truncated tool input or an unusable working directory. On this host it cannot block a call when the failure leaves the adapter with no output at all. If that residual risk matters in your environment, use [sandboxing](/guides/vs-sandboxing) as the containment layer.
