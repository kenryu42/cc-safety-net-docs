...
CC Safety Net analyzes the command string your agent attempts to run, so it cannot catch everything — for example behavior hidden inside arbitrary binaries, transparent command proxies, reads and network activity, and a few parser edge cases. See [Known Limitations](/guides/known-limitations) for the full list and the recommended mitigations.
...
<Note>
  For the system component view and data-flow diagrams, see [Architecture](/guides/architecture). For a deep dive on each analyzer and the `rm -rf` classification hierarchy, see [Analysis Engine](/guides/analysis-engine). For the design rationale behind this approach, see [Design Principles](/guides/design-principles).
</Note>