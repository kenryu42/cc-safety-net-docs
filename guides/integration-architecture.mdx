...
For these eight agents, the protection is the runtime `hook <flag>` command, which reads JSON from stdin. Each agent names its pre-execution event and its command tool differently, and each expects a different deny shape on stdout:
...
CC Safety Net writes files directly for five of the eight agents. Four use hook configuration entries, and Hermes Agent uses a managed plugin. The other three use their own plugin or extension distribution channel, which then invokes the stdin hook:
