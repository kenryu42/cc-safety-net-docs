...
`bun run check` is the single gate. It runs, in order: Biome lint and format, TypeScript typecheck, `knip` dead-code detection, `jscpd` duplicate detection, the `ast-grep` pattern scan, residual-risk registry validation, the test suite with coverage, and a coverage-threshold check. Run it once when you finish your changes rather than running the sub-commands separately. Make sure it passes with no errors before opening a pull request.
