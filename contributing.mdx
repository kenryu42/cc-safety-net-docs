...
`bun run check` is the single gate. It runs, in order: Biome lint and format, TypeScript typecheck, `knip` dead-code detection, `jscpd` duplicate detection, the `ast-grep` pattern scan, residual-risk registry validation, the test suite with coverage, and a coverage-threshold check. Run it once when you finish your changes rather than running the sub-commands separately, and make sure it passes with no errors before opening a pull request.
...
| Build output | Ignore `dist/` — the husky pre-commit hook rebuilds it |
...
- Tested locally with at least one supported agent — for example Codex, Claude Code, Gemini CLI, GitHub Copilot CLI, Kimi Code, or Pi. All ten are listed on the [Installation](/installation#install-a-specific-agent) page.
...
- Review existing implementations in `src/core/` for code patterns, and `tests/helpers.ts` for test utilities.
