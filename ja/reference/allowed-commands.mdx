...
CC Safety Net はコマンドの意味を解析し、許可できる形式と破壊的な形式を区別します。このページにあるコマンド形式は、アナライザーが許可します。
...
このページは、許可されるコマンドのリファレンスです。ブロックされる形式は[ブロックされるコマンド](/ja/reference/blocked-commands)、ガードの処理順は[アーキテクチャ](/ja/guides/architecture)、分類方法は[解析エンジン](/ja/guides/analysis-engine)を参照してください。
...
| `git checkout <branch>` | 素のブランチ名はパスの復元ではなく、ブランチの切り替えとして扱う |
| `git checkout -d` / `--detach`、`-t` / `--track` | これらのオプションが付くと、オペランドはパス復元の判定から除外される |
...
素のブランチ名が許可されるのは、パスの形をしておらず、解決後の git の作業ディレクトリに実在するエントリも指していない場合だけです。実在するファイルやディレクトリと同じ名前のブランチはブロックされ、拒否メッセージは `git switch` を案内します。パスの形については[ブロックされるコマンド](/ja/reference/blocked-commands#git-commands)を参照してください。
...
| `rm -rf "$target"` | 実行前に検証できない再帰削除の対象 |
| `Remove-Item $target -Recurse -Force` | 同じ検証できない対象を使う PowerShell の同等形式 |
...
| `f() { rm -rf "$1"; }; f ~` | 呼び出された関数本体では位置パラメーターが未束縛のままであるため、対象が動的になる。`rm -rf "$target"` と同じトレードオフ |
...
- 先頭のベース名が、リモート取得コマンド（`curl`、`wget`、`fetch`、`aria2c`、`http`、`https`、`xh`、`xhs`、`nc`、`ncat`、`netcat`）でも、シェルでも、`sudo` や `env` のような標準的なコマンドラッパーでもない

`CMD` 自体は引き続き解析するため、`eval "$(rm -rf /)"` はブロックします。検証*しない*のは `CMD` が出力するシェルです。つまり standard は、リテラルなローカルコマンドでありさえすれば、その出力を信頼します。`eval "$(cat somefile)"` のような形も含まれます。ブロックされたままの形については、[生成されたコマンドの eval と source](/ja/reference/blocked-commands#eval-and-source-of-a-generated-command)を参照してください。
...
| `dd if=x.iso of=./out.img`、`dd if=/dev/urandom of=random.dat` | 出力先が通常のファイル |
| `mkfs.ext4 disk.img`、`mkfs.ext4 ./loop.img` | `/dev/` デバイスではなく、ファイルベースのイメージをフォーマットする |
| オペランドがない `shred` | `shred` ルールには少なくとも 1 つの対象が必要。`shred --help` と `shred --version` は、すべてのオペランドが対象になるため**ブロックされる** |
| `ldd ./bin`、`ddrescue if=/dev/sda of=./out.img` | ワード境界で一致する。`dd` または `shred` を部分文字列として含むだけの先頭コマンドは、ルールが対象とする先頭コマンドではない |
...
| `Remove-Item /tmp/test-dir -Recurse -Force` | `rm` と同じ分類の一時ディレクトリの対象。設定済みの allow path も同様に動作する |
| `Remove-Item 'file''name.txt'` とバッククォートでエスケープした形式 | 破壊的な対象に解決されない、引用またはエスケープされた単純削除 |
...
シェルモードに `posix` を選ぶと、PowerShell の削除ルールは意図的にすべて無効になります。一方、`git.reset-hard` や `rm.recursive-force-root-or-home` などのクロスシェルルールは維持されます。`auto` モードでは、明示的な `Remove-Item` が検出されます。これは、`;`、改行、`&&`、`||` の後にある場合も同じです。`Get-Content`、`Set-Content`、`Add-Content`、`Copy-Item`、`Move-Item` も同様に検出され、`gc` や `cp` のような別名も引数が PowerShell のパス式で書かれていれば検出されます。
...
| 機密パスのリテラルを含むインタープリターのインラインコード | コードにファイルシステム操作、コマンド実行、eval のいずれの痕跡もない場合、リテラルは実行されないデータのままになる。POSIX シェルのコードは対象外で、ガードがシェルとしてたどる | **standard のみ** |
...
- `git checkout -- <file>`、`git checkout <path>`、`git checkout <ref> -- <file>`、`git checkout --force`、位置引数が複数あって判別しにくい checkout の形式
...
後述の一時ルートの緩和はこれとは別の経路です。オペランド自体が信頼された一時ルート配下の使い捨て worktree ディレクトリであれば、`git worktree remove --force` が許可されることがあります。
...
`git.push-*` を除くすべての Git ルールが、この経路で緩和され得ます。対象となるリポジトリは、解決後の git の作業ディレクトリから最も近い、`.git` エントリを持つ祖先ディレクトリです。このリポジトリのルートは、信頼された一時ルートの配下にあり、一時ルートそのものではなく、ワークスペース（元のカレントディレクトリ）の祖先でも配下でもない必要があります。さらに `.git` が実在するディレクトリでなければならず、`.git` が存在しない場合やシンボリックリンクの場合はルールがそのまま適用されます。
...
詳しい条件は[解析エンジン](/ja/guides/analysis-engine#temp-root-relaxation)を参照してください。
