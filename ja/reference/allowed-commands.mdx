...
| `rm -rf ./subdir`（カレントディレクトリ配下） | 対象がカレントディレクトリ内に限定される | [paranoid rm](/ja/configuration/modes) 以外のすべて |
...
デリミタは引用符で囲む必要があります（`<<'EOF'`）。引用符のない `<<EOF` では本文の中で展開が起きうるため、standard では引き続きスキャンの対象になり、fail closed が有効な場合は拒否されます。heredoc の外にあるコマンドは、これまでどおり解析されます。`cat <<'EOF' && rm -rf ~` は `rm` によってブロックされます。`cat`、`tee`、`git commit`、`gh pr create`、`gh issue create` では、機密パスを抽出する前に本文もマスクされます。そのため、シークレットのファイル名を文章の中で触れただけでコミットがブロックされることはありません。`git apply` の本文は、patch が書き込むファイル名を含むため、パス抽出からは見える状態のままにします。条件の全体と、条件を満たさない heredoc を standard がどう扱うかについては、[heredoc の解析](/ja/guides/analysis-engine)を参照してください。

## standard のみの許可

次の形式は standard では許可され、[strict](/ja/configuration/modes) または [paranoid](/ja/configuration/modes) を有効にすると拒否されます。これは見落としではなく、意図的なトレードオフです。standard は、敵対的な入力や動的な入力に対してはベストエフォートという位置づけだからです。
...
[`CC_SAFETY_NET_WORKTREE=1`](/ja/configuration/modes) を設定すると、CC Safety Net は Git の linked worktree であることを確認したうえで、一部のローカル破棄コマンドを許可します。確認できなかった場合、そのコマンドはブロックされたままです。
