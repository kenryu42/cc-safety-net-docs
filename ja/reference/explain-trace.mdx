...
description: "cc-safety-net explain --json が返す JSON のスキーマ。ExplainResult フィールド、各解析手順を示す TraceStep のバリエーション、共有前にトレースが明らかにする情報を説明します。"
keywords:
  - "explain"
  - "explain --json"
  - "トレース"
...
`heredoc-file` は、内容を解析がすでに把握しているスクリプトファイルをコマンドが実行した場合に記録されます。その内容とは、同じコマンド内で先に `cat >` または `tee` を使ってそのパスへ書き込んだ、引用符付き heredoc 本文です。保存済みの本文はスクリプトとして再解析されます。[heredoc 解析](/ja/guides/analysis-engine)を参照してください。
