...
- `Rule config synced.`、空行、`Active rulebooks (<n>):`。各エントリは `  - <name> <version> (<n> rules)` と `    Source: <spec>`

裸のローカル名または正規形式 `owner/repo#ref/<rulebook-name>` のソースでは、最初の 3 行は出力されません。リポジトリ追加で `Rule config synced.` が出る位置には、`Added rulebook source: <source>` が出力されます。
...
実行のたびにブランチとタグの ref を解決し直すため、`main` や移動するタグを指しているソースは現在のコミットを取り込みます。ローカルソースには取得するものがありません。設定済みのソースはすべてレポートに現れますが、再取得されるのは選択したソースだけで、残りはディスク上のファイルから読み込まれます。出力される変更ブロックは [`rule add`](#rule-add) と同じで、その後に `Rule config synced.` とアクティブな rulebook の一覧が続きます。
...
`Active rules` では、どのルールも `  Command:`、`  Block args:`、`  Reason:` の同じ 3 行を出力します。バージョン 1 のルールは、サブコマンドを設定していれば `Command:` の行に `<command> <subcommand>` を表示し、ブロック対象の引数を `Block args:` に表示します。`rulebook_version: 2` のルールにはどちらもないため、`Command:` の行はコマンド名だけになり、`Block args:` の行は空になります。バージョン 2 の `match` の内容は `rule list` には出力されません。`match.command_path`、`match.any_args`、`match.exclude_args` は rulebook ファイル自体を読むか、[`rule doc`](#rule-doc) で作成ガイドを表示して確認してください。
