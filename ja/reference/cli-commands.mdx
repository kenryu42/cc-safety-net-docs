...
<Warning>
  explain の出力は、そのまま安全に共有できるとは限りません。入力したコマンド、解析後のトークン、ホームディレクトリを含む絶対パスがそのまま表示されます。Issue やチャットにトレースを貼り付ける前に、[explain トレース](/ja/reference/explain-trace)を確認してください。
</Warning>
