...
使用方法は `rule add <source> [--ref <ref>] [--only <rulebook...>]` です。`<source>` には 3 つの形式があります。`project-rules` のような裸のローカル名、`acme/safety-rules` のようなリポジトリ全体、`owner/repo#ref/<rulebook-name>` という正規形式で指定する単一の rulebook です。
...
ソースを省略するとエラーになります。他のサブコマンドと同様に、`-g` / `--global` でユーザースコープを選択します。
...
裸のローカル名または正規形式 `owner/repo#ref/<rulebook-name>` のソースでは、最初の 3 行は出力されません。リポジトリ追加で `Rule config updated.` が出る位置には、`Added rulebook source: <source>` が出力されます。
