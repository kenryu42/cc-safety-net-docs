...
ソースを指定せずに `--ref` または `--only` を渡すと、公式カタログの `cc-safety-net/rulebooks` に解決されます。どちらのフラグもない `rule add` は `rule add requires a source (pass --only <rulebook...> to select from cc-safety-net/rulebooks)` を出力して `1` で終了します。そのため、公式 rulebook をすべて追加する場合は、これまでどおり `rule add cc-safety-net/rulebooks` と明示する必要があります。
...
`rule.json` には、指定した ref を含む正規形式 `owner/repo#ref/<rulebook-name>` が保存されます。解決されたコミットは出力されるだけで、保存はされません。
