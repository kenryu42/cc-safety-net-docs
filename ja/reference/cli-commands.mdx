...
| フラグ | エージェント | インストール方法 |
| --- | --- | --- |
| `--amp` | Amp Code | `~/.config/amp/plugins/cc-safety-net.ts` にプラグインファイルを書き込む |
...
| `--kimi-code` | Kimi Code | `~/.kimi-code/config.toml`（または `$KIMI_CODE_HOME/config.toml`）に `[[hooks]]` ブロックを書き込む |
...
- **プラグインファイル** — Amp Code と Hermes Agent。変更後は Amp を再起動するか `plugins: reload` を実行します。Hermes Agent では、インストール時に `hermes plugins enable` も実行され、変更後に Hermes の再起動が必要です。
...
- **フラグなしの対話型ターミナル：** 矢印キーを使う複数選択プロンプトが表示されます。各ターゲットの使用可否を調べます。そのため、エージェントがインストールされていない場合は `CLI not installed`、設定済みの場合は `already installed`、uninstall 時に未設定の場合は `not installed` と表示されます。
...
選択したターゲットは、選択した順序ではなく、常にカタログのインストール順で実行されます。失敗すると、権限問題、パス不足、ディレクトリではないパス要素など、エラー固有のヒントとともに `1` で終了します。
...
ターゲットは、各エージェントの設定ファイルと状態ファイルを読み取って検出します。現在無効な場合でも、インストール済みと検出される統合が対象です。GitHub Copilot CLI は例外です。Copilot の `installed-plugins` ディレクトリに CC Safety Net プラグインの checkout がある場合だけ対象になります。無効状態と、何もインストールされていない kill-switch 状態を区別できず、update が install になることを防ぐためです。Claude Code または Codex でレガシーの `safety-net@cc-marketplace` プラグイン ID を使うインストールも検出され、更新時に現在の ID へ移行されます。

各ターゲットは、カタログのインストール順で `install` と同じ操作を実行し、メッセージだけが `Updated …` または `… up to date` に変わります。インストール時にエージェント自身の CLI を使うターゲット、つまり Claude Code、Codex、GitHub Copilot CLI、Gemini CLI、Hermes Agent、OpenClaw、OpenCode、Pi では、先にベンダーバイナリを調べます。バイナリがない場合は `<Agent> not found; skipped`（例は `Codex not found; skipped`）を出力して処理を続けます。設定ファイルまたはプラグインファイルを使うターゲット、つまり Amp Code、Antigravity CLI、Cursor、Kimi Code にはバイナリが不要で、常に更新されます。

対象がない場合、`update` は ``No installed integrations found. Run `cc-safety-net install` to set one up.`` を出力して `0` で終了します。

`update` はターゲットフラグも引数も受け取りません。使用できるのは `-h` / `--help` だけです。ほかのオプションは `Unknown option for update: <flag>`、位置引数は `Unexpected argument for update: <arg>` を出力し、どちらも `1` で終了します。ターゲットの更新に失敗すると `install` と同じエラーヒントとともに `1` で終了します。それ以外の終了コードは `0` です。
