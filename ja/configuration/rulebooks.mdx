...
ルールを無効化したりブロック理由を差し替えたりするには、`rule.json` のルール単位の上書きを使います。rulebook ファイル自体は変更されないため、`rule update` と競合しません。構文は [`rule.json` のスキーマ](/ja/configuration/custom-rules#rule-json-schema)を参照してください。rulebook ごと削除するには：
...
`npx -y cc-safety-net rule list` は有効な各 rulebook とそのルール数を表示し、`npx -y cc-safety-net explain "terraform destroy"` は判定がどのルールによるものかをトレースします。設定元が無効と表示される場合の修復手順は、[rulebook はライブファイル](/ja/configuration/custom-rules#rulebooks-are-live-files)を参照してください。
