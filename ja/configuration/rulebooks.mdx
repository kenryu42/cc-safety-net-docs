...
`--only` と `--ref` はリポジトリのソースでしか意味を持たないため、ソースを指定せずにどちらかを渡すと、`cc-safety-net/rulebooks` が選ばれます。リポジトリを自分で指定しても、これまでどおり動作します。このリポジトリが公開する rulebook をすべて導入する場合は、この指定が必要です。
