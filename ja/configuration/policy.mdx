...
sidebarTitle: "ポリシー"
description: "CC Safety Net の policy.json の完全なリファレンスです。場所、schema、安全 preset と機能 override、worktree mode、破壊的コマンドとシークレットの保護、deny path と allow path の rule、audit retention、既定値、優先順位を説明します。"
---
...
<ParamField body="secret_protection.allow_paths" type="string[]" default="[]">
  組み込みの secret pattern rule から除外する正確な file または directory tree です。設定済みの deny path と Coding CLI protection は引き続き適用します。validation rule と優先順位については、[Secret allow path](#secret-allow-path)を参照してください。
</ParamField>
...
`secret_protection.allow_paths` は、正確な file または directory とそのすべての descendant を、組み込みの basename、home-directory、key-variant、extension rule から除外します。repository の `.env.test` や、secret ではない credential 形式の filename を含む fixture directory など、利用者が意図して管理する file に使用します。

優先順位は固定です。

1. 設定済みの deny path が常に優先します。同じ target が両方の list にある場合、結果は `secret.deny-path` です。
2. Coding CLI credential rule と config rule（`secret.cli.*`）が常に優先します。allow path は agent 自身の credential または configuration を公開できません。
3. allow path に一致すると、その target に対するその他の組み込み secret rule を抑制します。
...
| --- | --- |
| string ではない、または trim 後に空になる string | 無効 — `must be a non-empty path string` |
| `.env.test` や `fixtures/credentials` などの relative path | **有効** — session ごとに config working directory を基準に解決 |
| `*` または `?` を含む項目 | 無効 — `cannot contain glob characters (* or ?); list the exact file or directory` |
| `~`、`$HOME`、`${HOME}` だけ、または home より上位の path | 無効 — `cannot cover the home directory or a path above it (this would disable secret protection everywhere)` |
| `~/.cc-safety-net` またはその配下 | 無効 — `cannot cover the guard's own configuration` |
| その他の literal absolute path、`~/…`、`$HOME/…`、`${HOME}/…` | 有効 |

項目は literal path であり、glob pattern ではありません。target は execution working directory を基準に解決し、relative allow entry は config working directory を基準に解決します。same-or-descendant comparison の前に両側の既存 symlink を解決するため、allow root は symlink 経由の target も対象にします。

runtime は解決後に safety boundary を再確認します。home または home の ancestor に解決される項目は無視します。有効な guard configuration root 配下の target は除外しません。`CC_SAFETY_NET_HOME` で root を移動した場合や、home または `~/.cc-safety-net` が symlink の場合も同じです。
...
| `secret_protection.allow_paths` | 無効な項目を破棄。array ではない場合は `[]`。secret exemption なし |
