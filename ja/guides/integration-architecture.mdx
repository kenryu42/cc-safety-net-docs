...
| Kimi Code | 直接書き込み：`npx -y cc-safety-net hook --kimi-code` を実行する `[[hooks]]` block を `~/.kimi-code/config.toml`（または `$KIMI_CODE_HOME/config.toml`）に追加。もう 1 つの方法として、Kimi Code 内で `/plugins install https://github.com/kenryu42/cc-safety-net` を実行して native Kimi Code plugin を install できます。manifest は同じ hook entry（`node ./dist/bin/cc-safety-net.js hook --kimi-code`、`PreToolUse` event、timeout 30 秒）を matcher なしで実行するため、file tool も hook に到達します |

Kimi Code の `Bash` call では、`tool_input.cwd` がある場合、その値を execution directory として使います。値は session `cwd` の内側に解決できる空でない string である必要があります。無効な値や外側に出る値は fail closed します。GitHub Copilot CLI は `powershell` と `PowerShell` の call を PowerShell analyzer に route します。
...
ここでは、2 つの directory detail が重要です。`terminal` call では、plugin は最初に Hermes の session ごとの cwd record を読み取ります。この record は session の `cd` state を保持します。最初の command で record がまだない場合は、`TERMINAL_CWD`、次に Hermes process directory を使います。使用できない `workdir` を持つ `terminal` call は fail closed します。analyzer subprocess は Hermes working directory ではなく home directory から開始します。このため、`npx` が正規版の代わりに repository-local `cc-safety-net` を解決することはありません。
...
plugin は `exec` tool に一致する `before_tool_call` handler を登録します。解析するのは tag のない shell `exec` だけです。Code Mode の JavaScript `exec` など、`toolKind` discriminator がある `exec` event には判定を返しません。その `command` field が shell command を表すことは証明されていないためです。他の OpenClaw tool は guard に渡しません。handler は判定なし（allow）または `{ block: true, blockReason }` を返し、tool parameter を書き換えません。

OpenClaw runtime API で agent workspace directory を解決し、policy directory と execution directory の両方に使います。call 内の `workdir` は workspace 内に contained するよう解決します。CC Safety Net は、malformed event、missing または empty command、解決できない workspace、workspace 外の `workdir`、すでに cancelled の call で fail closed します。`host` が `auto` または `gateway` 以外の `exec` call も block します。`gateway` は local であることが証明されています。`host: "auto"`（または `host` なし）の call は local Gateway semantics で解析しますが、plugin は OpenClaw が実際に route する場所を確認しません。sandbox の場合は[既知の制限](/ja/guides/known-limitations)を参照してください。
...
`bash` tool では、plugin は OpenCode の `shell` setting から analyzer dialect を選びます。setting が string でない場合、Windows では PowerShell を default とし、それ以外では `SHELL` を使います。認識した `powershell` と `pwsh` executable は PowerShell を選び、認識した POSIX shell は POSIX を選びます。他の値では自動検出を使います。指定された `workdir` は、read と search ができる directory に解決できる場合だけ execution directory になります。Windows では、documented `/C:`、`/C`、`/cygdrive/C`、`/mnt/C` 形式を normalize し、他の slash-rooted path は OpenCode が解決できるようそのまま渡します。
...
1. **`tool_call` event handler を登録します**（`pi.on('tool_call')`）。handler は Pi tool call を実行前に検査し、command または path が policy に違反する場合は block result を返します。process boundary を越えるものはありません。
2. Pi 内で rulebook を対話的に管理する **`/cc-safety-net` builtin command を登録します**。

### Pi が保護する tool

Pi の command-tool adapter は組み込み **`bash`** tool だけです。その `command` は session cwd に対して実行されます。古い custom `Shell` adapter はサポートしません。Pi の non-command tool は引き続き path protection と secret protection に到達します。特に、`find` は read-only glob tool として分類されるため、search 自体を write として扱わずに `pattern` と path value を検査します。command call または session cwd が malformed の場合、CC Safety Net は fail closed します。
