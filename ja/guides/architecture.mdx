...
CC Safety Net の runtime package dependency は **`zod` 1 つだけ**で、configuration validation のみに使います。`createRequire` で遅延読み込みするため、configuration を実際に validate する場合だけ cost が発生します。Node bundle と Pi bundle では external のままにして、installed package から解決します。import する source module は正確に 1 つです。
