...
使用只含排除项的无害 pathspec 确认已知阻止行为：`git checkout -- ':(exclude,top)**'` 必须被阻止。即使保护未启用，此 pathspec 也不会选择任何文件。
...
- 为新规则添加测试，覆盖率至少为 90%。
- 至少使用一个受支持的智能体进行本地测试，例如 Codex、Claude Code、Gemini CLI、GitHub Copilot CLI、Kimi Code 或 Pi。[安装](/zh-Hans/installation#安装指定智能体)页面列出了全部十二个智能体。
