...
<Tip>
要快速编写自定义规则，可在智能体中使用 **`/cc-safety-net` 技能**，通过自然语言创建规则。例如：
...
- **活动 rulebook 名称重复时，以先声明者为准。** 用户范围先加载，因此会遮蔽同名的项目 rulebook。后一个 rulebook 的所有规则都不生效。名称冲突会产生警告，并使运行时进入 `degraded`。请重命名其中一个 rulebook，再运行 `rule sync`。名称冲突本身不会使 `rule sync` 失败。
...
- **本地源偏离其固定摘要**并不算故障状态，包括在磁盘上丢失、无法解析或不符合 schema 的情况。运行时只读取经过摘要校验的**缓存** rulebook，绝不会重新读取本地副本。在你运行 `rule sync` 之前，本地未同步的改动不会生效。完整的 `rule sync` 和 `rule verify` 仍会拒绝无效的或符号链接的本地源。
...
<Warning>
`transparent_wrappers` 位于 `rule.json` 中，而 `rule.json` 不带 lock 或摘要。如果某个范围的 `rule.json` 变得不可读，该范围的包装器就不再生效。这是配置被丢弃会削弱内置覆盖范围的唯一情况。被丢弃的 **rulebook**（缺少缓存条目、缺少 lock 条目、摘要不匹配）不影响 `rule.json` 的可读性，因此包装器不受影响。
</Warning>
...
无法通过校验的自定义规则配置会被**丢弃并停止生效**，不会因此阻止命令。普通命令照常运行，其他已校验的源和所有内置保护继续生效。运行时会报告 `degraded`。
