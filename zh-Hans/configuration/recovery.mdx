...
逐字段保留意味着一个无效字段不会丢弃文件其余部分配置的保护。保护性默认值会优先产生更多拒绝：强制开启破坏性命令保护和机密保护，丢弃两个允许路径列表中的无效条目，并丢弃用于关闭规则的覆盖。有效的机密允许路径保留在挽救的策略中。每个字段的保留行为见[策略](/zh-Hans/configuration/policy)。
