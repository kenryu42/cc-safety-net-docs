...
`npx -y cc-safety-net rule list` 显示每个活动 rulebook 及其规则数量，`npx -y cc-safety-net explain "terraform destroy"` 可以追踪某个决策出自哪条规则。如果某个源显示为未激活，修复步骤见 [rulebook 是实时文件](/zh-Hans/configuration/custom-rules#rulebook-是实时文件)。
