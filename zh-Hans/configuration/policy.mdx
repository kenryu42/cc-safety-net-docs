...
从内置机密模式规则中豁免的精确文件或目录树。配置的拒绝路径和 Coding CLI 保护仍然适用。验证规则和优先顺序见[机密允许路径](#机密允许路径)。
</ParamField>
...
`secret_protection.allow_paths` 从内置的 basename、主目录、密钥变体和扩展名规则中豁免精确文件或目录及其所有后代。将它用于您有意管理的文件，例如存储库的 `.env.test`，或包含非机密凭据形式文件名的 fixture 目录。

优先顺序是固定的：

1. 配置的拒绝路径始终优先。如果同一目标在两个列表中，结果是 `secret.deny-path`。
2. Coding CLI 凭据和配置规则 (`secret.cli.*`) 始终优先。允许路径不能公开智能体自己的凭据或配置。
3. 允许路径匹配会抑制该目标的任何其他内置机密规则。
...
| --- | --- |
|非字符串，或清理为空的字符串 |无效 — `must be a non-empty path string` |
|相对路径，例如 `.env.test` 或 `fixtures/credentials` | **有效** — 根据配置工作目录解析每个会话 |
|包含 `*` 或 `?` 的条目 |无效 — `cannot contain glob characters (* or ?); list the exact file or directory` |
|单独的 `~`、`$HOME` 或 `${HOME}`，或 home 之上的路径 |无效 — `cannot cover the home directory or a path above it (this would disable secret protection everywhere)` |
| `~/.cc-safety-net` 或其下的任何路径 |无效 — `cannot cover the guard's own configuration` |
|任何其他字面绝对路径、`~/…`、`$HOME/…` 或 `${HOME}/…` 路径 |有效 |

条目是字面路径，而不是 glob 模式。目标根据执行工作目录解析，相对允许条目根据配置工作目录解析。在进行相同或后代比较前，两边都会跟随现有符号链接，因此允许根目录也涵盖通过符号链接到达的目标。

运行时会在解析后再次检查安全边界。解析为 home 或 home 祖先的条目会被忽略。有效防护配置根目录下的目标永远不会被豁免，包括 `CC_SAFETY_NET_HOME` 移动该根目录，或 home 或 `~/.cc-safety-net` 是符号链接时。
...
| `secret_protection.allow_paths` |无效条目被丢弃；非数组变为 `[]` — 没有机密豁免 |
