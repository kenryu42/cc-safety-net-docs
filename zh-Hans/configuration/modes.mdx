...
`standard` 还会有意允许动态可执行文件、通过替换组装的命令结构、其他无法验证的递归删除目标，以及对内置敏感路径仅读取元数据的独立检查。这些是有意的权衡：对于对抗性或动态输入，`standard` 仅提供**尽力保护**。如果命令可能来自提示注入或其他对抗性环境，请使用 `strict` 或 `paranoid`。
