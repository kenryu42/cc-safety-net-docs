...
否则，安装会运行 `opencode plugin add cc-safety-net@latest` 和 `opencode plugin update cc-safety-net@latest`，再读取 `opencode plugin list`，要求其中有一行的包 ID 为 `cc-safety-net`、来源为 `cc-safety-net@latest`。缺少该行则失败：
