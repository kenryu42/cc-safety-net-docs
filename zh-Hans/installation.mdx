...
此命令把托管插件构件 `cc-safety-net.ts` 发布到你账户的 Amp Personal Plugins 仓库。个人插件跟随账户生效，因此也能保护在远程计算机上执行的线程，例如 Amp Orb。

安装需要 `amp` CLI 和已登录的账户。安装程序先运行 `amp plugins repositories --json`，查找你可写的 Personal Plugins 仓库。缺少 CLI 或未登录时，安装会停止并提示运行 `amp login`。然后，安装程序将 `user-plugins` 克隆到一次性临时目录，写入 `cc-safety-net.ts`，以工具身份 `cc-safety-net` 提交，再推送。提交时会关闭签名，避免全局 git 配置阻塞安装。重新运行命令会更新已发布的构件。
