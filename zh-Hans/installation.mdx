...
所有安装命令都是幂等的。对已配置的智能体再次运行命令是安全的，最终只会保留一个托管项。
...
该项不带 matcher，因此到达适配器的不只是 `run_terminal_command`，而是每一次工具调用；适配器还会检查文件类和 patch 类工具输入中的受保护路径。
...
两条命令都只改动托管的 handler。安装先从已有项中剥离托管 handler，再把规范项作为独立的一项追加进去，因此非托管的项、与托管 handler 同处一项的非托管 handler，以及其他 hook 事件都原样保留。卸载只移除托管 handler；某一项移除后不再有 handler，该项也一并删除。整个文件只有在其中没有其他内容时才删除。无法解析的文件在卸载时原样保留；安装则会把它修复为规范内容，因为 Grok Build 会整个跳过无法解析的 hook 文件，这样的文件本来也无法承载可用的 hook。

<Note>
  Grok Build hook 是故障时放行，宿主也没有提供 `failClosed` 开关。只有明确的 deny 才能阻止工具调用；hook 崩溃、超时或输出格式错误时，调用照常执行。
</Note>
