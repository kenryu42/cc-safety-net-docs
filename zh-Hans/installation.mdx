...
如果你在 2.2 或更早的版本上从 GitHub 安装过 rulebook，升级后需要按范围各运行一次 [`rule sync`](/zh-Hans/reference/cli-commands#rule-sync)，用户范围的源加上 `--global`。rulebook 现在是落盘后的实时文件，不再依赖 lock 和缓存。该命令会把每个缓存的 rulebook 迁移到其来源实际加载的实时路径，然后删除残留文件。在此之前，来自 GitHub 的这些规则处于未生效状态，`status` 和 `doctor` 会把这些来源报告为 degraded。
