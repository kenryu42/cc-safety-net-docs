...
大多数集成支持 **Windows、macOS 和 Linux**。它们会检测主机操作系统，以处理路径和解析命令。在 Windows 上，这包括不区分大小写的路径，以及通过 `COMSPEC` 和 `PATHEXT` 解析 `cmd.exe` 或 PowerShell。
...
`update` 也是主要版本之间的升级路径。在 v1 安装上运行它会将每个已安装的集成迁移到当前 v2 版本。如果你在 v1 内联配置中定义了自定义规则，请完成[旧配置迁移](/zh-Hans/configuration/custom-rules)。然后运行 `npx cc-safety-net doctor`，确认运行时为 `ready`。
...
此命令将 npm 软件包中的自包含插件构件复制到 `~/.config/amp/plugins/cc-safety-net.ts`，因此不需要托管插件、URL 或全局 npm 依赖。重新运行命令会原地更新托管构件。
...
如果 `~/.config/amp/plugins/cc-safety-net.ts` 是符号链接、目录，或是不含 CC Safety Net 托管标头的普通文件，安装和卸载都拒绝修改它。请自行移动或删除该文件，然后重新运行命令。

插件只保护其安装所在的计算机。在 Amp Orb 等远程执行器上执行的工具调用不受保护。
...
`doctor` 会对每个受支持的智能体运行完整健康检查，包括 hook 集成、确认阻止有效的自检、自定义规则验证、活动模式标志、近期活动、系统版本和更新检查。有关每项检查和可用参数，请参阅 [doctor 命令参考](/zh-Hans/reference/cli-commands)。
