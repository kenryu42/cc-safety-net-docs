...
本页是面向维护者的防护管线规范。<a href="/zh-Hans/guides/how-it-works">工作原理</a>提供用户视角。<a href="/zh-Hans/guides/integration-architecture">集成架构</a>说明各智能体如何接入防护。
...
- 适配器只向精确匹配的、集成专用的工具名授予**命令执行能力**。未知工具不会按 shell 命令处理。它仍接受策略文件、Git 元数据和敏感路径检查，但其文本不会解析为命令。
...
- 结果**深度不可变**。策略对象、规则数组、每条规则及其 `block_args`、透明包装器列表、安全块及其 override、破坏性命令规则 override、allow path、机密保护块及其 disabled rule 和 deny path 都已冻结，快照包装器本身也是如此。
...
| 9 | `secret-protection` | **敏感路径保护**：检查 command、path、search 和 patch 形式中的内置敏感路径及配置的 deny path。匹配时以 `hard_stop` intent 拒绝，并携带匹配的 rule ID。策略禁用机密保护时完全跳过。 | 是 |
| 10 | `non-command` | 通过阶段 1–9 的非命令 invocation 在此获得允许。 | — |
...
- **阶段 6 和 7 在阶段 8 之前运行。** 策略文件保护和 Git 元数据保护会在加载任何配置之前作出拒绝。它们始终启用、不带 config state，任何 preset、override 或 master switch 都无法削弱它们。
...
分析器的公开约定有意保持狭窄：允许时不返回内容，阻止时返回结果对象。它不会公开解析器内部的 `complete` / `partial` / `limited` 状态，因此调用方无法根据解析置信度作分支判断。
...
已发布 bundle **不包含第三方 shell 解析器**。路径扫描器读取的 flat entry stream 由 `projectShellSyntax` 从已解析 IR 投影。因此，上述内部 POSIX 和 PowerShell 解析器是 shell 结构的唯一来源，不存在可能与之产生偏差的第二次原始命令文本 tokenization。
