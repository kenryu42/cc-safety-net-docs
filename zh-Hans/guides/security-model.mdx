...
启用机密保护时，安全级别不会放宽已匹配敏感**内容**的访问，也不会放宽用户配置的 deny path 及其后代。配置的 `secret_protection.allow_paths` 条目可以从非 CLI 内置机密规则中豁免字面文件或目录树；拒绝路径和 `secret.cli.*` 规则仍然优先。灾难性保护始终执行，包括递归删除根目录或用户主目录、破坏性更改受保护 Git 元数据，以及破坏性更改规范用户 `policy.json`。
...
| **敏感文件访问** | 通过 command、path、search 或 patch 形式读取或发现 `.env`、`~/.ssh/id_*` 或 `~/.aws/credentials` 等凭证 | 敏感路径保护覆盖受支持 command、path、search 和 patch 形式，并检查未知工具 fallback；用户配置的 deny path 及其后代最先匹配且绝不放宽；字面机密允许路径可以豁免非 CLI 内置匹配，但 Coding CLI 路径仍然受保护；strict 和 paranoid 还会阻止仅元数据发现。覆盖范围是有界模式集，不是通用读取边界 |
