...
<AccordionGroup>
  <Accordion title="Hook 未运行，命令未经检查就执行">
    ...
    **解决步骤：**
    ...
    3. **Amp Code**：确认 `~/.config/amp/plugins/cc-safety-net.ts` 存在且仍有 CC Safety Net 托管标头。Amp 在启动时读取插件，因此更改后请重启 Amp 或运行 `plugins: reload`。
    ...
    13. **OpenCode**：检查 `~/.config/opencode/opencode.json`（或 `.jsonc`）的 `plugin[]` 是否包含 `cc-safety-net`。旧缓存的处理步骤见[安装](/zh-Hans/installation)。
