...
<Note>
  包的根导出是 OpenCode 插件对象，不是引擎。`checkCommand` 请从 `cc-safety-net/api` 导入。使用 OpenCode v1 SDK 时，从 `cc-safety-net` 导入 `CCSafetyNetPlugin` 或默认插件的 `server`；使用 v2 SDK 时，用 `cc-safety-net/opencode/v2` 的默认导出。每个入口的类型声明只需要各自那一代的 optional peer。
</Note>
