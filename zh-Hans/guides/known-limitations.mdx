...
OpenClaw 插件只为规范 `exec` 工具注册 `before_tool_call`。`apply_patch` 和 OpenClaw 的 read/write/edit 文件工具不受保护。带 `toolKind` 区分标记的 `exec` 事件也不会得到判定。这会排除与 shell 工具共用 `exec` 名称的工具，例如 Code Mode 的 JavaScript `exec`，因为尚未证明其 `command` 字段是 shell 命令。
