...
对于 Kimi Code `Bash` 调用，如果存在 `tool_input.cwd`，它会成为执行目录。该值必须是非空字符串，并解析在会话 `cwd` 内；无效或越界的值会 fail closed。GitHub Copilot CLI 会把 `powershell` 和 `PowerShell` 调用路由到 PowerShell 分析器。
...
两个目录细节很重要。对 `terminal` 调用，插件先读取 Hermes 的每会话 cwd 记录，其中保存会话的 `cd` 状态。第一个命令执行时，如果该记录尚不存在，则依次使用 `TERMINAL_CWD` 和 Hermes 进程目录。`terminal` 调用带有不可用的 `workdir` 时会 fail closed。分析器子进程从主目录启动，而不是 Hermes 工作目录，因此 `npx` 不会用仓库本地 `cc-safety-net` 替代真实软件包。
...
插件为 `exec` 工具注册 `before_tool_call` handler。只分析没有标签的 shell `exec`。带 `toolKind` 区分标记的 `exec` 事件（如 Code Mode 的 JavaScript `exec`）不会得到判定，因为尚未证明其 `command` 字段是 shell 命令。不转发其他 OpenClaw 工具。Handler 返回无判定（允许）或 `{ block: true, blockReason }`，绝不改写工具参数。

插件通过 OpenClaw runtime API 解析智能体 workspace，并把它同时用作策略和执行目录；调用中的 `workdir` 必须解析在 workspace 内。格式错误事件、缺失或空命令、无法解析 workspace、workspace 外的 `workdir` 或已取消调用都会 fail closed。`host` 不是 `auto` 或 `gateway` 的 `exec` 也会阻止。`gateway` 已证明为本地执行。`host: "auto"` 或无 `host` 的调用按本地 Gateway 语义分析，但插件不检查 OpenClaw 实际路由位置。沙箱情况见<a href="/zh-Hans/guides/known-limitations">已知限制</a>。
...
Pi 的命令工具适配器只支持内置 **`bash`** 工具。其 `command` 相对会话 cwd 执行。已不支持旧的自定义 `Shell` 适配器。Pi 的非命令工具仍会进入路径保护和机密保护。其中，`find` 被分类为只读 glob 工具，因此会检查其 `pattern` 和路径值，但不会把搜索本身当作写入。命令调用或会话 cwd 格式错误时，CC Safety Net 会 fail closed。
