...
| [`rule`](#rule) | `rule <subcommand>` | 管理规则配置、rulebook 来源和透明包装器 |
...
| `Level` | 生效级别：`standard`、`strict`、`paranoid` 或 `custom`；当任何生效的破坏性命令规则与该级别本应继承的规则不同时，会带上 ` (customised)` 后缀 |
...
| Configuration | 验证用户和项目规则配置，并列出生效的规则 |
| Environment | 显示 CC Safety Net 环境变量的状态 |
| Effective Safety | 显示选定的预设、生效级别、能力和规则覆盖，包括任何削弱继承规则的覆盖 |
...
有关旧版布局和当前布局的区别，请参阅[审计日志](/zh-Hans/reference/audit-log#log-layout)。
...
`rule` 管理规则配置、rulebook 来源和透明命令包装器。本节说明命令接口；rulebook 的 schema、生命周期和覆盖语义见[自定义规则](/zh-Hans/configuration/custom-rules)。
...
| `--delete-source` | 移除来源时，若本地来源目录是干净的，则一并删除 | 仅 `remove` |
| `--example` | 创建一个非活动的示例 rulebook | 仅 `init` |
| `--ref <ref>` | 指定分支、标签或提交 | 仅 `add` |
| `--only <rulebook...>` | 只添加仓库中这些 rulebook | 仅 `add` |
| `-h`, `--help` | 显示帮助 | 任意子命令 |

CLI 不再接受 `--check`。任何子命令都会以 `Unknown option for rule: --check` 拒绝它。rulebook 在每条命令执行时都从磁盘读取，因此 `add` 或 `update` 的试运行要有意义，就必须先拉取并校验候选内容。离线校验请用 [`rule verify`](#rule-verify)。
...
用法是 `rule add [source] [--ref <ref>] [--only <rulebook...>]`。来源有三种形式：`project-rules` 这样的纯本地名称、`acme/safety-rules` 这样的整个仓库，或 `owner/repo#ref/<rulebook-name>` 规范形式指定的单个 rulebook。
...
省略来源时，只要带上 `--ref` 或 `--only`，来源就解析为官方目录 `cc-safety-net/rulebooks`。两个选项都不带的 `rule add` 会打印 `rule add requires a source (pass --only <rulebook...> to select from cc-safety-net/rulebooks)` 并以 `1` 退出。要添加官方的全部 rulebook，仍需显式写出 `rule add cc-safety-net/rulebooks`。
...
- `Vendored at <commit>.`，即解析出的提交，缩短为 7 个字符，仅在本次添加至少写入了一个新来源时才打印
...
纯本地名称或规范形式 `owner/repo#ref/<rulebook-name>` 的来源保留 Scope 这一行，但不打印上面的前三项，结尾也不是 `Rule config updated.`，而是 `Added rulebook source: <source>`。
...
- 若并发进程在这段间隙里新增了文件，第二次检查会拒绝删除并报错：`Local rulebook source directory contains extra files: <dir>. delete manually if you really want to remove the directory.` 随后配置改动会回滚并重新同步，你要求移除的来源又会回来。
...
为所有已配置的来源重新拉取并落盘远程 rulebook；给出单个来源时，只处理该来源：
...
每次运行都会重新解析分支和标签 ref，因此指向 `main` 或某个会移动的标签的来源会取到当前提交。本地来源没有要拉取的内容。所有已配置的来源都会出现在报告里，但只有选中的来源会重新拉取，其余的从磁盘上已有的文件读取。打印的变更块与 [`rule add`](#rule-add) 相同，随后是 `Rule config updated.` 和活动 rulebook 列表。
...
全程不联网。缓存中仍与所记录摘要一致的副本会写入 `<config-dir>/<name>/rulebook.json`，并报告 `Vendored <spec> from the v2 cache.`；如果目标文件存在但不可用，则报告 `Restored <spec> from the v2 cache over an invalid file.`。缓存无法提供的来源会打印 ``Could not migrate <spec> from the v2 cache. Run `cc-safety-net rule update <spec>` to vendor it.``，在用户范围下该命令还会带上 ` --global`。最后一行是 `Removed the v2 lock and cache under <dir>.`。
...
列出用户范围和项目范围内活动的 rulebook 及其解析出的来源：
...
| `--opencode` | OpenCode | 运行 OpenCode 自身的插件命令：1.x 用 `opencode plugin -g -f`，2.x 用 `opencode plugin add` 和 `opencode plugin update` |
| `--pi` | Pi | 运行 Pi 的包安装 |
...
- **原生插件或扩展命令**：Claude Code、Codex、GitHub Copilot CLI、Gemini CLI、OpenClaw、OpenCode 和 Pi。CC Safety Net 调用智能体自带的插件管理器，并顺带清理已被取代的插件 ID。OpenClaw 的安装在完成后还会验证 OpenClaw 是否报告插件已加载，然后请你重启 OpenClaw Gateway。在 OpenCode 1.x 上，安装会导入缓存的包条目，并验证它导出了可调用的 `CCSafetyNetPlugin`；在 2.x 上，安装会读取 `opencode plugin list`，要求其中有一行 `cc-safety-net` 且来源为 `cc-safety-net@latest`。任一检查失败都会中止安装，因为 OpenCode 否则会什么都不加载并 fail open。
...
Amp Code、OpenClaw、OpenCode 和 Pi 没有各自的 `hook` 标志。它们把 CC Safety Net 作为插件或扩展加载到进程内。有关每个智能体的连接方式，请参阅[集成架构](/zh-Hans/guides/integration-architecture)。
