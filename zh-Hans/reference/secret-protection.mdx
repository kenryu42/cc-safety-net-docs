...
title: "秘密保护参考"
sidebarTitle: "秘密保护"
description: "CC Safety Net 内置秘密保护规则的完整目录：敏感基本名称、受保护的主目录、密钥文件变体、凭据扩展、两个编码 CLI 层、拒绝路径、允许路径以及所有豁免。"
keywords:
  - "秘密保护"
  - "敏感路径"
  - "凭据文件"
...
内置匹配不会把远程 URL、搜索正则表达式，或只以敏感形式前缀开始的说明文本当作本地路径。`file:` URL 是本地路径，仍然受保护。带空格的文件名在该路径存在于磁盘上时也仍然受保护。在已识别的 CC Safety Net `explain` 调用中，被说明的命令是数据，不会被视为文件操作数。

## 匹配顺序

每个候选路径都会按顺序经过以下检查。第一个确定的结果优先；豁免仅适用于其后的规则。

1. **配置的[拒绝路径](/zh-Hans/configuration/policy)** — 在每个内置规则和每个豁免之前检查。命中归因于 `secret.deny-path`。
2. **Env 模板豁免** — 模板基本名称保持可读；请参阅[豁免](#exemptions)。
3. **[主目录规则](#home-directory-rules)** — 整个受保护的目录，例如 `~/.ssh`。这些规则优先于公钥豁免，因此不在机密允许路径中的 `~/.ssh/id_rsa.pub` 仍然会被阻止。
4. **[编码 CLI 规则](#coding-cli-credential-tier-on-by-default)** — 每个智能体的凭据和配置路径。
...
内置规则匹配后，配置的[机密允许路径](/zh-Hans/configuration/policy#机密允许路径)会抑制该匹配，除非规则 ID 以 `secret.cli.` 开头。拒绝路径已在步骤 1 中优先，Coding CLI 规则永远不会被放宽。
...
它们会同时比较字面主目录相对路径和经过文件系统解析的路径。`~/.ssh` 或 `~/.aws` 等凭据目录在是符号链接时仍然受保护，字面比较前会折叠 `..` 段。
...
[`secret_protection.allow_paths`](/zh-Hans/configuration/policy#机密允许路径) 从内置模式规则中豁免精确文件或目录树。拒绝路径仍然优先，`secret.cli.*` 保护永远不会被豁免。条目只能是字面路径。目标和允许根目录会在比较前跟随现有符号链接，但任何有效的覆盖 home 的根目录都会被拒绝，有效 CC Safety Net 配置根目录下的目标仍然受保护。完整验证表见[策略](/zh-Hans/configuration/policy#机密允许路径)。
...
- `secret_protection.allow_paths` 在检查拒绝路径后，从非 CLI 内置规则中豁免精确文件或目录树。

破坏性命令的[允许路径](/zh-Hans/guides/analysis-engine)和任何标准模式的允许都不会放宽机密保护。只有 `secret_protection.allow_paths` 具有上述受限效果。在[严格模式](/zh-Hans/configuration/modes)下，受保护路径的仅元数据发现（`test -f ~/.ssh/id_rsa`、`find ~/.ssh -type f`）也会被阻止。[仪表板](/zh-Hans/guides/dashboard)通过 UI 编辑相同的设置。
