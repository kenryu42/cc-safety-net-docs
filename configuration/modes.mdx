...
Note that invalid hook input JSON, analyzer exceptions, and parser resource-limit failures are **already fail-closed in every mode** — those are not what strict adds.
...
The `paranoid` level is strict plus two further capabilities, `paranoid_rm` and `paranoid_interpreters`. These checks may be disruptive to some normal workflows, so they are opt-in. You can select the whole level, or activate the individual capabilities selectively — a level that carries only one of them reports as the effective level `custom`.
...
Setting `CC_SAFETY_NET_PARANOID=1` is equivalent to enabling both `CC_SAFETY_NET_PARANOID_RM=1` and `CC_SAFETY_NET_PARANOID_INTERPRETERS=1` simultaneously. It does **not** turn on `fail_closed`, so on top of the default standard level it produces the effective level `custom` rather than `paranoid`. Use `CC_SAFETY_NET_LEVEL=paranoid` (or `safety.level: "paranoid"`) when you want the full preset.
...
| Variable | Effect |
| --- | --- |
| `CC_SAFETY_NET_LEVEL=standard\|strict\|paranoid` | Select a preset; raises the policy level but never lowers it |
| `CC_SAFETY_NET_STRICT=1` | Fail-closed capability: unparseable commands, unverifiable destructive targets, metadata-only sensitive-path discovery |
| `CC_SAFETY_NET_PARANOID=1` | Both paranoid checks (rm \+ interpreters); does not set fail-closed |
| `CC_SAFETY_NET_PARANOID_RM=1` | Block non-temp `rm -rf` even within cwd |
| `CC_SAFETY_NET_PARANOID_INTERPRETERS=1` | Block every interpreter one-liner regardless of content |
| `CC_SAFETY_NET_WORKTREE=1` | Relax local-discard rules in linked worktrees |

See [Environment Variables](/configuration/environment) for the legacy `SAFETY_NET_*` aliases and the full precedence rules between `policy.json` and the environment.
