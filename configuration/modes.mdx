...
<Note>
  Mode and debug flags use `CC_SAFETY_NET_*` environment variables. Older `SAFETY_NET_*` names (without the `CC_` prefix) are also accepted for strict, paranoid, paranoid-rm, paranoid-interpreters, and worktree toggles. See the [environment variable reference](/configuration/environment) for the full list, including `CC_SAFETY_NET_DEBUG` and `CC_SAFETY_NET_HOME`.
</Note>

## Default Mode

No environment variables are required to get started. In default mode, CC Safety Net protects against all built-in destructive git and filesystem patterns. This is the recommended starting point for most users.

Most malformed input is already handled safely regardless of mode:
...
<Tip>
  You can check which modes are currently active by running `npx cc-safety-net doctor` or by looking at the status line in Claude Code. See [Status Line](/configuration/status-line) for setup instructions.
</Tip>