...
These commands affect shared refs or other worktrees, so **worktree mode never relaxes them**:
...
A separate [temp-root relaxation](/guides/analysis-engine#temp-root-relaxation) can allow `git worktree remove --force` when the operand is a disposable worktree directory under a trusted temp root. It is not part of worktree mode and does not need `CC_SAFETY_NET_WORKTREE=1`.
