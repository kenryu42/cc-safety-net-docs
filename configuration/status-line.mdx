...
The ⚠️ marker is the only passive signal that configuration is degraded. It does not say which source failed — run `npx cc-safety-net status` for the diagnostics, or `npx cc-safety-net doctor` for the full report. See [Configuration recovery](/configuration/recovery) for what `ready` and `degraded` mean and how to repair each one.
