...
With `CC_SAFETY_NET_HOME` set, the file sits **directly** under that directory, as a sibling of `rules/`. See [Environment variables](/configuration/environment) for the override itself.
...
- **The dashboard.** `cc-safety-net gui` gives you a form over the same document, writes it with the correct permissions, and can repair a file that no longer validates. It is the lower-risk option, and the only one that repairs a broken file for you.
...
<ParamField body="secret_protection.overrides" type="object" default="{}">
  Per-rule state, keyed by registered secret-protection rule id. The **only** accepted value is `"off"` — there is no `"on"`, because every secret rule is on whenever secret protection is enabled.
</ParamField>
...
Resolution runs in this order, so later steps win:

1. The **preset** from `safety.level` supplies inherited capability defaults.
2. **`CC_SAFETY_NET_LEVEL`** raises the base level, and never lowers it.
3. **`safety.overrides.*`** sets each named capability explicitly, up or down.
4. **Boolean environment flags** — `CC_SAFETY_NET_STRICT`, `CC_SAFETY_NET_PARANOID`, `CC_SAFETY_NET_PARANOID_RM`, `CC_SAFETY_NET_PARANOID_INTERPRETERS` — each force their capability to `true`. They only raise.

`worktree_mode` is a logical OR of the policy field and `CC_SAFETY_NET_WORKTREE`: either one enables it.

<Note>
  Because the boolean flags are monotonic, an environment variable can never weaken your policy file, and your policy file can never weaken an environment variable that is already set. See [Environment variables](/configuration/environment) for the full list, including legacy `SAFETY_NET_*` aliases.
</Note>
...
Secret protection blocks reads and writes of credential-bearing files. `secret_protection.overrides` disables individual built-in rules by id, and `"off"` is the only accepted value:
...
An unregistered id is rejected with `unknown secret protection rule id "<id>"`, and any other value with `secret_protection.overrides.<id> must be "off"`.
...
<CardGroup cols={2}>
  ...
  <Card title="Environment variables" icon="variable" href="/configuration/environment">
