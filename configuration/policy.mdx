...
To find the id for a blocked command, read it from the block message or run `npx -y cc-safety-net explain --json '<command>'` and read the `ruleId` field. Every id, what it blocks, and when it is active is listed in the [Destructive-command rules reference](/reference/destructive-rules), which also walks through [allowing a built-in rule](/reference/destructive-rules#allow-a-built-in-rule).

**Catastrophic rules are always enforced and are not user-configurable.** They ignore `enabled: false` and ignore an `"off"` override. These are the six rules covering removal of `/` or your home directory and deletion of Git metadata, together with their PowerShell and `find` equivalents. They are marked in the [reference table](/reference/destructive-rules#catastrophic-rules).
