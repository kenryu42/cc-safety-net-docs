...
Secret protection blocks reads and writes of credential-bearing files. `secret_protection.overrides` addresses individual built-in rules by id, with the value `"on"` or `"off"`. `"off"` disables a rule that is on by default; `"on"` opts into a rule from the default-off tier:
...
Most built-in secret rules are on whenever secret protection is enabled. One tier is not: the **Coding CLI config** rules, which cover the settings and MCP configuration files of supported coding agents. Those files can carry credentials inline, but agents also edit them as routine work, so the tier ships off and you opt in per rule with an explicit `"on"` override. The ten default-off ids:

- `secret.cli.claude-code.config`
- `secret.cli.codex.config`
- `secret.cli.gemini.config`
- `secret.cli.copilot-cli.config`
- `secret.cli.kimi-code.config`
- `secret.cli.opencode.config`
- `secret.cli.pi.config`
- `secret.cli.amp.config`
- `secret.cli.cursor.config`
- `secret.cli.antigravity`

Each of those agents also has a **Coding CLI credential** rule covering its credential stores — `secret.cli.claude-code`, `secret.cli.codex`, `secret.cli.gemini`, `secret.cli.copilot-cli`, `secret.cli.kimi-code`, `secret.cli.opencode`, `secret.cli.pi`, `secret.cli.amp`, `secret.cli.cursor` — and those are on by default like every other secret rule. Antigravity is the exception: `secret.cli.antigravity` is its only rule, it sits in the default-off config tier, and it covers just `~/.gemini/config/hooks.json` and `~/.gemini/config/mcp_config.json`.

A config rule you turn on protects the agent's user-level config files, and it also protects project-level files matched by name rather than location: any `.mcp.json`, a `settings.local.json` inside a `.claude` directory, `opencode.json` or `opencode.jsonc` anywhere, `mcp.json` inside a `.cursor` or `.kimi-code` directory, `settings.json` inside a `.gemini` directory, and `settings.json` or `settings.jsonc` inside a `.amp` directory.
