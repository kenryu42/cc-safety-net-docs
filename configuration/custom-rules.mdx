...
Either way the runtime reports itself as `degraded` rather than blocking work. See [Recovery](/configuration/recovery) for the full state model, the exact diagnostic strings, and the repair sequence.
...
<ParamField body="rules" type="array">
  List of rulebook source strings. Defaults to an empty array. Source names must be unique within the file.
</ParamField>
...
Legacy rules are never enforced from their old location, and they never block work either: the legacy source is simply dropped and named in the diagnostics. This is the quietest failure class, because dropping a source only removes denials — nothing rubs. Check `npx cc-safety-net status` after an upgrade.
...
| Scenario | Behavior |
| --- | --- |
| Config file not found | Silent — built-in rules only, state stays `ready` |
| Malformed, empty, or unsupported-`version` `rule.json` | That whole scope is dropped, **including its `transparent_wrappers`** |
| Empty legacy config | Silent — built-in rules only |
| Legacy config with rules and no migrated rule config | Legacy rules are inert until `rule migrate` runs |
| Invalid legacy config | Inert until fixed or removed |
| Missing lockfile, missing lock entry, missing cache entry, or digest mismatch | Only that source is dropped; run `rule sync` |
| Invalid cached rulebook | Only that source is dropped |
| Local rulebook missing, invalid, or drifted from its digest | Warning only — the digest-verified **cached** rulebook keeps enforcing until you run `rule sync` |
| Duplicate active rulebook name | The first claim wins, user scope first; the later rulebook contributes nothing |
| Unknown override key, or a project override naming a user-scoped rule | Only that override is ignored |

Because a dropped source **removes** denials rather than adding them, this class of failure produces no friction on its own. `npx cc-safety-net status` is the cheap habitual check; `npx cc-safety-net doctor` gives the full report, and `npx -y cc-safety-net rule list` exits non-zero when any issue or warning remains. See [Recovery](/configuration/recovery) for the diagnostic strings, the reporting surfaces, and the repair sequence.
