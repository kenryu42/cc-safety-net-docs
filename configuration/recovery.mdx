...
| `cc-safety-net status` | The verdict `ready`, `degraded`, or `not enforcing`, and one line per diagnostic under `Not active` |
...
<Steps>
  <Step title="Check the verdict">
    ...
    Prints `ready`, `degraded`, or `not enforcing`, plus one line per diagnostic under `Not active`. It is purely informational, so use it as the cheap habitual check rather than a gate.
  </Step>
  ...
  <Step title="See what is actually active">
    ...
    Lists what is actually active, followed by `Issues` and `Warnings`. Use it to confirm which rules a dropped source took with it.
  </Step>
  <Step title="Validate your rule configuration">
    ...
    Validates the user and project `rule.json` against the schema and replays the runtime load, so it catches the same problems the guard would hit. It also flags legacy files that still need migration.
  </Step>
