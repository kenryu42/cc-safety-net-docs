...
| A lockfile entry does not match its configured source identity because it has the wrong `kind`, or a `path`/`name` differs from the source spec | Every rulebook in that scope. The whole lockfile is rejected as malformed | The other scope's verified rules and all built-ins |
...
<Warning>
  The local rulebook source is deliberately absent from both tables. Runtime load reads only the digest-verified cache, so editing a local rulebook, breaking it, or deleting its source directory produces **no warning at all**. The cached rulebook from the last successful sync remains active, not the file you just edited. Your edit becomes active only when `cc-safety-net rule sync` succeeds.
</Warning>
...
<Steps>
  ...
  <Step title="Validate your rule configuration">
    ...
    Validates the user and project `rule.json` against the schema and replays the runtime load, so it catches the same problems the guard would hit. It also flags legacy files that still need migration. The command can make one change. When a valid `rule.json` has no `$schema` key, `rule verify` adds one and prints `Added $schema to <scope> config.` Otherwise, it changes nothing.
  </Step>
  <Step title="Repair and re-sync">
    ...
    Rewrites the lockfile and cache for the scope you are syncing, then reloads that scope exactly as the guard loads it. If any diagnostic remains, it reports that diagnostic and exits non-zero instead of claiming success. `Rule config synced.` means that the scope has no remaining diagnostic.
