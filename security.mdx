...
<Note>
  A destructive-command coverage gap stays public even when it looks severe. It is not reclassified as a vulnerability, and the private path is reserved for the categories listed above. Note also that a command allowed under [standard mode's documented relaxations](/guides/security-model#what-each-safety-level-guarantees) is not a gap at all — standard is best-effort, and strict or paranoid is the level that fails closed on those forms.
</Note>
